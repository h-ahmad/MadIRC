...
class Feautre_Descriptor(abc.ABC):
    ...
    def generate_descriptors(self, images:np.ndarray, quite: bool = False ):
        with torch.no_grad():
            ...
            for _, image in enumerate(tqdm.tqdm(images, ncols=100, desc = 'Gen Feature Descriptors', disable=quite)):  
                ...
                # print('len(features): ', len(features)) # 13, 
                # print('features.keys(): ', features.keys()) # dict_keys(['Level_1', 'Level_2', 'Level_3', 'Level_4', 'Level_5', 'Level_6', 'Level_7', 'Level_8', 'Level_9', 'Level_10', 'Level_11', 'Level_12', 'Level_13'])
                features = [features[layer] for layer in features.keys()]
                # print('features shape: ', features[0].shape) # [1, 256, 56, 56], [1, 256, 60, 60]
                features = [self.patch_maker.patchify(x, return_spatial_info=True) for x in features]
                patch_shapes = [x[1] for x in features]
                features = [x[0] for x in features]
                ref_num_patches = patch_shapes[0]
                # print('features[0]: ', features[0].shape) # 56, 60
                # print('\n patch_shapes[0][0]: ', patch_shapes[0][0]) # [1, 3136, 256, 3, 3], [1, 3600, 256, 3, 3]
                # print('len(features): ', len(features)) # 13, 
                ...
                # print('features[0]: ', features[0].shape) # torch.Size([3136, 256, 3, 3]),  torch.Size([3600, 256, 3, 3])
                features = self.agg_preprocessing(features)
                features = self.pre_adapt_aggregator(features)
                # print('features.shape: ', features.shape) # torch.Size([3136, 1024]), torch.Size([3600, 1024]
    ...
    def image_net_norm(self, image: np.ndarray):
        ...
        # print('\nimage_1: ', image.shape) # (224, 224, 3), (240, 240, 3)
        image = np.expand_dims(image, axis=0)
        image = np.transpose(image,axes=[0,3,1,2])
        if len(image.shape) == 3:
            image = np.expand_dims(image,axis=0)
        # print('image: ', image.shape) # (1, 3, 224, 224), (1, 3, 240, 240)
